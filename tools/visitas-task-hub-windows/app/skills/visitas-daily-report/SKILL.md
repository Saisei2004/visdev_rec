...
```powershell
py -3 "$env:LOCALAPPDATA\OneFPSRecorder\app\reporting\reportctl.py" --report-config
py -3 "$env:LOCALAPPDATA\OneFPSRecorder\app\reporting\reportctl.py" --report-candidates (Get-Date -Format yyyy-MM)
```
...
Windowsでは既定がdry-runである。外部書き込みを伴う実行は各宛先の個別許可を確認した後だけ行い、未許可の宛先を `false` にする。

```powershell
py -3 "$env:LOCALAPPDATA\OneFPSRecorder\app\reporting\reportctl.py" --submit-report-json C:\path\request.json --dry-run
```
