...
~/Movies/1FPS録画/YYYY-MM/業務報告-YYYY-MM.md
...
- 月ごとの `業務報告-YYYY-MM.md` に、指定日の報告を追加または上書き
- 業務時間は録画区間ログから計算し、切り捨て1時間単位で記録
- 業務動画リンク欄には、Drive共有リンクを取得後に貼り付け可能
- 提出先Driveフォルダは標準で `松田彩成` に設定され、提出後に自動で開きます
...
- `業務報告初期値...`: 提出画面で最初に入る担当者、業務プラン、業務内容、次回Task、状態、メッセージ、DriveフォルダURLを変更
